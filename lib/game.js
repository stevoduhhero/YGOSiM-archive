...
global.gameCount = 0;
global.lastSave = 0;
var gameCountPath = path.join(__dirname, "./gameCount.txt");
try {
	gameCount = parseInt(fs.readFileSync(gameCountPath)) || 0;
	lastSave = gameCount;
...
function Game(p1, p2, tier, rated, p1Deck, p2Deck) {
	...
	(function() {
		...
		if (difference === 10) {
			lastSave = gameCount;
			fs.writeFile(gameCountPath, '' + gameCount);
		}
	})();
...
Game.prototype.receive = function(user, data) {
	switch(data.event) {
		...
		case 'counter':
			...
			if (!player || !data.type || !(data.zone >= 0 && data.zone < 10)) return;
