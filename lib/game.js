...
gameCount = db.get('gameCount').value() || 0;
gameCountSaveDifference = 1; //how many games must elapse before updating .txt w/ gameCount
lastSave = gameCount;

//add starter decks to variable
fs.readdir("./lib/starter decks", function(err, files) {
	for (var i in files) {
		var file = files[i];
		if (!file.endsWith('.dek')) continue;
		var deckString = fs.readFileSync("./lib/starter decks/" + file) + '';
...
function Game(p1, p2, tier, rated, p1Deck, p2Deck) {
	...
	if (difference === gameCountSaveDifference) {
		lastSave = gameCount;
		db.set('gameCount', gameCount).write();
	}
...
Game.prototype.importDeck = function(deckString) {
	...
	*/
	...
	if (typeof deckString !== "string") return deck;
