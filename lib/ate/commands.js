global.tour = {
	...
	start: function(user, room, tier, toursize) {
		...
		Tour.prototype.end = function(user) {
			if (user) this.room.addRaw('<h2>The tournament was ended by ' + htmlescape(user.getIdentity()) + '.</h2>');
			delete this.tours[this.room.id];
		};
...
var commands = {
	...
	tour: function(data, room, user, socket) {
		...
		if (part[0] !== "advanced" && part[0] !== "traditional") return socket.send(room, "You did not enter a valid tier.[ADVANCED, TRADITIONAL]");
