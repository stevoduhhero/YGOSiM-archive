...
var events = {
	...
	'search': function(user, data, socket) {
		...
		function addToSearch(user, data, socket) {
			...
			if (!data.tier) return;
			if (toId(data.tier) !== "random" && !data.deckString) return;
			data.deck = Game.prototype.verifyDeck(data.tier, data.deckString);
		...
		if (data.tier) {
			data.tier = toId(data.tier);
			if (data.tier === "random") data.deckString = Game.prototype.randomDeck();
		}
		if (data.deck && !data.deckString) {
			//provided deck name and no deckString
			//get deckString by name and complete this function
	...
	/* ate */
	'rooms': function(user, data, socket) {
		var userCount = Object.keys(users).length;
		var gameCount = Object.keys(games).length;
		var _rooms = '';
		for (var roomid in rooms) {
			//TODO: if not staff, hide staff room (Room.staff ? yes : no)
			var room = rooms[roomid];
			_rooms += Object.keys(room.users).length + '~' + room.title + '~' + room.desc + '|';
		}
		_rooms = _rooms.slice(0, -1);
		socket.send('rooms|' + userCount + '|' + gameCount + '|' + _rooms);
	},
